...
nw_size = [5] #, 12, 17, 22]
algos = ['qtable', 'dqn']
algos_label=['Tabular Q-learning', 'DDQN (proposed)']
...
for ii in range(len(algos)):
    patch = mpatches.Patch(color=color[ii], label= str(algos_label[ii]))
...
for c in range(len(algos)):
    ...
    if algos[c] == 'qtable':
        ...
        # with open('results_comparison_new/qtable_var_nw5_beta0.0_coeff0.8.pkl', 'rb') as f:
        ...
        with open('results_comparison_new/dqn_reproduce_var_nw5_beta' + str(0.0) + '_coeff0.8.pkl', 'rb') as f:
            save_reward, save_goodput, save_connect_ratio, save_energy, save_txr = pickle.load(f)
            total_reward = save_reward
            total_goodput = save_goodput
            total_connect_ratio = save_connect_ratio
    ...
    for i in range(total_goodput.shape[0]):
        ...
        goodput_dataframe = pd.DataFrame({"timestep": [tt for tt in range(total_goodput.shape[1])], "goodput": total_goodput[i], 'hue': color[c]})#, ignore_index=True)
        connect_dataframe = pd.DataFrame({'timestep': [tt for tt in range(total_goodput.shape[1])], "connect": total_connect_ratio[i], 'hue': color[c]})#, ignore_index=True)
        enery_dataframe = pd.DataFrame({'timestep': [tt for tt in range(total_goodput.shape[1])], "energy": total_energy[i], 'hue': color[c]}) #,ignore_index=True)
        goodput_data = goodput_data.append(goodput_dataframe)
        connect_data = connect_data.append(connect_dataframe)
        enery_data = enery_data.append(enery_dataframe)
...
# plt.show()
plt.savefig('sup_qtable_dqn_goodput2.pdf')
...
# plt.show()
plt.savefig('sup_qtable_dqn_cr2.pdf')
...
# plt.legend(handles=beta_patches)
plt.legend(handles=nw_patches)
plt.savefig('sup_qtable_dqn_energy2.pdf')
