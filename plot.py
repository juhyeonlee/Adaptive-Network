...
nw_patches = []
for ii in range(len(init_range)):
    patch = mpatches.Patch(color=color[ii], label='Initial range=' + str(init_range[ii]))
...
for c in range(len(init_range)):

    if c == 0:
        with open(
                '../Adaptive-Network-exps/amazon/20180206_nwsize_exp_10ep/var_nw' + str(nw_size[0]) + '_beta' + str(
                        0.0) + '_coeff0.8.pkl', 'rb') as f:
        ...
        with open('../Adaptive-Network-exps/amazon/20180209_nwsize_exp_10ep/var_nw' + str(
                nw_size[0]) + '_beta0.0_coeff0.8.pkl',
                  'rb') as ff:
        ...
        with open('results_comparison_new/dqn_init' + str(init_range[c]) + '_var_nw' + str(nw_size[0]) + '_beta' + str(0.0) + '_coeff0.8.pkl',
                  'rb') as f:
            save_reward, save_goodput, save_connect_ratio, save_energy, save_txr = pickle.load(f)
            total_reward = save_reward
            total_goodput = save_goodput
...
# plt.show()
plt.savefig('sup_init_goodput.pdf')
...
# plt.legend(handles=beta_patches)
plt.legend(handles=nw_patches)
plt.savefig('sup_init_energy.pdf')
